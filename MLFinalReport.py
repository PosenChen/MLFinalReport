...
(4)pooling的size  直接設定
"""
#%% 載入套件&超參數設定區  超參數僅需設定 CRPF 以大寫英文字母CRPF字串方式設定 並修改第164 165行程式碼後 就可以執行全部的程式碼 再自行手動input參數
...
if os.path.exists(csv_file_path):
    df3.to_csv(csv_file_path, mode='a', header=False, index=False, encoding='utf-8-sig')
else:
    df3.to_csv(csv_file_path, mode='w', header=True, index=False, encoding='utf-8-sig')
